...
{
    /// <summary>
    /// MCP tool for getting parameter information for stored procedures across databases.
    /// </summary>
    [McpServerToolType]
...
- get_stored_procedure_parameters(""dbo.sp_UpdateCustomer"", format: ""table"")
- get_stored_procedure_parameters(""sp_ProcessPayment"", ""PaymentDB"", ""json"")
...
JSON format provides JSON Schema with SQL-specific extensions for parameter validation and documentation.")]
            ...
            int? timeoutSeconds = null)
        {
            ...
            try
            {
                ...
                if (!parameters.Any())
                {
                    return $"Stored procedure '{procedureName}' has no parameters or does not exist in database '{databaseName ?? "current"}'.";
                }

                return format.ToLower() switch
                {
                    "json" => FormatParameterInformationAsJson(procedureName, parameters, databaseName),
                    "table" => FormatParameterInformation(procedureName, parameters, databaseName),
        ...
        /// <summary>
        /// Gets stored procedure parameter metadata from the database.
        /// </summary>
